...
graphics.off()
source("cfg.r")
load("data/gfed4s_2000_2010_fordoug.RData")
load("data/mcd45_2001_2010_fordoug.RData")
load("data/meris_2005_2010_fordoug.RData")

BAs = list(GFED4 = gfed_ba, GFED4s = gfed4s_ba,
           "GERADO" = gerado_ba, "MCD45" = mcd45_ba, "MERIS" = meris_ba, "FIRE_CCI5.1" = ba_modis)
...
cummalitivePlot <- function(vars, name, bin, yaxis = FALSE, yaxis2 = FALSE,
                            cummm = TRUE, poly = FALSE, logit = FALSE, diffFrom1st = TRUE) {
    if (logit) yrange = c(-7.7, 7.7) else yrange = c(0, 1)
    
    plot(range(bins), yrange, type = 'n', xaxt ='n', yaxt = 'n', 
...
doThePlot <- function(fname, ...) {
    png(fname, height = 7, width = 9, units = 'in', res = 300)
        ...
        mapply(cummalitivePlot, varss, names(varss), bins, c(T, F, T, F), c(F, T, F, T), MoreArgs = list(...))
