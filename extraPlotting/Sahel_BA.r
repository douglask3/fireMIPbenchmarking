...
plotQs <- function(q, col = "black") {
    layers = 1:ncol(q)
    addPolygon <- function(i, border = NA) {
        qi = q[c(i, nrow(q) -i + 1),]*100
...
mask = raster::resample(mask, out[[1]][[1]])
mask = raster::crop(mask, extent)
mask = aa > 0.04 & mask > 20

png("figs/SahelMap.png", width = 3*2/3, height = 7*2/3, res = 300, units = 'in')
par(mar = rep(0,4))
map_cols = c('#ffffcc','#ffeda0','#fed976','#feb24c','#fd8d3c','#fc4e2a','#e31a1c','#bd0026','#800026')
...
#add_raster_legend2(dat = aa, cols = map_cols, limits = map_limits, transpose = F, srt = 0, plot_loc = c(0.1, 0.4, 0.09, 0.127), oneSideLabels=FALSE, extend_max = TRUE, units = '%')
