...
plotSpatialNmod <- function(dat, txt, index, limits, cols, range, scale) {
	...
	plotAgreement(mean(mod) * scale, txt[[1]][2], limits, cols, e = sd.raster(mod))
...
plotSeasonalNmod <- function(dat, txt, index, range) {
	...
	plotAgreement(nmod, txt[[2]][3])
	
	plot.new()
	plot.new()
	
	obsC = obs[[2]]
	plotAgreement(obsC, txt[[3]][1], SeasonConcLimits, SeasonConcCols)
...
for (r in range) {		
	...
	fname = paste('figs/nmodAgreement', '-R', r, '.png', sep = '')
